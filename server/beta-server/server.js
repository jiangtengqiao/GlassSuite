...
app.post('/apply', (req, res) => {
  ...
  if (pass) {
    ...
    key = genKey(existing);
    db.keys[key] = { email: a.email, issuedAt: new Date().toISOString(), active: true };
  }
  ...
  res.json({
    ...
    key,
    reasons,
    message: pass ? '申请通过！请保存你的尝鲜码（22~37 位）。' : '申请未通过：' + reasons.join('；'),
  });
});
...
app.post('/verify', (req, res) => {
  ...
  if (rec && rec.active) {
    res.json({ code: 200, valid: true, beta: true, email: rec.email });
  } else {
    res.json({ code: 200, valid: false, beta: false });
  }
});

app.get('/latest', (req, res) => {
  // 简化实现：由应用侧直接请求 GitHub Releases 并按权限过滤；
  // 此处仅作通道健康检查与权限判定返回。
  const key = String(req.query.key || '').trim();
  const db = load();
  const rec = key ? db.keys[key] : null;
  const beta = !!(rec && rec.active);
  res.json({ code: 200, beta, source: 'github' });
});
